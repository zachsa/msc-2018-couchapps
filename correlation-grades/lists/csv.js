function(head, req) {
    provides('csv', function() {
        /* Send the headers */
        var headers = "Year,StudentAnonID,Course,Course %,Gr12 Eng %,Gr12 Sci %,Gr12 Mth %,NBT AL %,NBT QL %,NBT Mth %,ALL";
        send(headers);
        ...
        while (row = getRow()) {
            ...
            switch (type) {
                case 'benchmark':
                    ...
                    currentLine["NBT AL %"] = value[3];
                    currentLine["NBT QL %"] = value[4];
                    currentLine["NBT Mth %"] = value[5];
                    currentLine["ALL"] = value[16];
                    break;
